...
export default function ChatPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setMe({ id: user.id, profileName: (user as any).profileName, username: (user as any).username, image: (user as any).image });
  ...
  // Realtime: استماع للرسائل الجديدة
  useEffect(() => {
    if (!me) return;
  ...
  const sendMessage = async () => {
    ...
    const { error } = await supabase.from("Message").insert([{ content, sender_id: me.id, receiver_id: receiverId }]);
    ...
    <div className="flex flex-col h-[100dvh] bg-slate-50 dark:bg-slate-950 overflow-hidden" dir="rtl">
      ...
      <header className="bg-white dark:bg-slate-900 p-3 px-4 flex items-center justify-between border-b dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-emerald-500 cursor-pointer" onClick={() => router.push(`/profile/${receiverId}`)}>
            {receiver?.image ? <img src={receiver.image} className="w-full h-full object-cover" /> : <span className="flex items-center justify-center w-full h-full bg-emerald-500 text-white font-bold">{receiver?.profileName?.[0]}</span>}
          </div>
      ...
      <div className="p-3 bg-white dark:bg-slate-900 border-t dark:border-slate-800">
        <div className="flex gap-2 items-center bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-[28px]">
          <input value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMessage()} placeholder="اكتب رسالة..." className="flex-1 bg-transparent p-2 px-4 outline-none text-[15px] dark:text-white" />
