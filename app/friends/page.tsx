...
export default function FriendsPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // جلب بيانات المستخدم الكاملة من جدول User
      ...
      if (userData) {
        setMe(userData);
        await fetchRelationships(userData.id);
      }
    };
    init();
  }, []);
  ...
  const handleFollow = async (targetId: string) => {
    ...
    try {
      ...
      await fetchRelationships(me.id);
    } catch (err) {
          ...
          return (
            <div key={person.id} className="bg-white p-4 rounded-[25px] flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-emerald-100 flex items-center justify-center font-bold text-emerald-600 overflow-hidden">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  ...
                  className={`px-4 py-2 rounded-xl font-bold text-xs transition-all ${iFollowHim ? "bg-slate-100 text-slate-500" : "bg-emerald-600 text-white"}`}
                  disabled={loading}
                >
