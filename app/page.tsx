...
export default function HomePage() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black text-emerald-600 animate-pulse text-2xl">REDA CHAT</div>;
  if (!user) return null;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24 transition-colors" dir="rtl">
      ...
      <main className="max-w-md mx-auto p-4">
          ...
          {/* 3. تاب قائمة الدردشات */}
          {activeTab === "chat" && (
            <div className="space-y-2">
              <h2 className="text-xl font-black mb-4 dark:text-white px-2">المحادثات</h2>
              {chatUsers.map(u => (
                <div key={u.id} onClick={() => router.push(`/chat/${u.id}`)} className="bg-white dark:bg-slate-900 p-4 rounded-2xl flex items-center gap-4 cursor-pointer hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-all border dark:border-slate-800">
          ...
          {activeTab === "profile" && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-slate-900 rounded-[35px] p-8 shadow-sm border dark:border-slate-800 text-center relative overflow-hidden">
                ...
                <button onClick={async () => { await supabase.auth.signOut(); router.push("/signup"); }} className="w-full mt-8 p-4 bg-red-50 dark:bg-red-900/10 text-red-500 rounded-2xl font-bold text-sm active:scale-95 transition-all">تسجيل الخروج</button>
              </div>
            </div>
          )}
      </main>
...
// مكون الزرار السفلي لسهولة الكود
function NavBtn({ icon, label, active, onClick }: any) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center transition-all ${active ? "text-emerald-600 scale-110 font-bold" : "text-slate-400"}`}>
